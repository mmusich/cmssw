...
void SiStripApproxClusters2Clusters::produce(edm::Event& e, edm::EventSetup const&) {
  ...
  for (edmNew::DetSetVector<SiStripApproximateCluster>::const_iterator i = clusterCollection->begin();
       i != clusterCollection->end();
       i++) {
    edmNew::DetSetVector<SiStripCluster>::FastFiller ff =
        edmNew::DetSetVector<SiStripCluster>::FastFiller(*result, i->id());
