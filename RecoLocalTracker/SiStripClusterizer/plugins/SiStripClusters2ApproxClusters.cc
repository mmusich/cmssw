...
void SiStripClusters2ApproxClusters::produce(edm::Event& e, edm::EventSetup const&) {
  std::unique_ptr<edmNew::DetSetVector<SiStripApproximateCluster>> result(
      new edmNew::DetSetVector<SiStripApproximateCluster>);

  edm::Handle<edmNew::DetSetVector<SiStripCluster>> clusterCollection;
  e.getByToken(clusterToken, clusterCollection);

  for (edmNew::DetSetVector<SiStripCluster>::const_iterator i = clusterCollection->begin();
       i != clusterCollection->end();
       i++) {
    std::vector<SiStripApproximateCluster> tempVec;

    edmNew::DetSetVector<SiStripApproximateCluster>::FastFiller ff =
        edmNew::DetSetVector<SiStripApproximateCluster>::FastFiller(*result, i->id());
