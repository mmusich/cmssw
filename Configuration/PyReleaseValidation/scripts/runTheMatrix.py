...
if __name__ == '__main__':
    ...
    predefinedSet['limited'] = (
        predefinedSet['limited_run1_run2'] +
        predefinedSet['limited_run3'] +
        predefinedSet['limited_phase2'] +
        predefinedSet['limited_heavyIons']
    )
    ...
    if opt.testList:
        ...
        for entry in opt.testList.split(','):
            ...
            for k in predefinedSet:
                if k.lower().startswith(entry.lower()) or k.lower().endswith(entry.lower()):
                    testList.extend(predefinedSet[k])  # Collect all matches
                    found_match = True  # Mark as matched but continue searching
        ...
        opt.testList = list(set(testList))  # Remove duplicates
