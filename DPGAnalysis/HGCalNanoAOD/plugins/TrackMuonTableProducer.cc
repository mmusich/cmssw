...
public:
  ...
  void produce(edm::StreamID, edm::Event& evt, const edm::EventSetup&) const override {
    auto tracksH = evt.getHandle(tracksToken_);
    auto muonsH = evt.getHandle(muonsToken_);

    const size_t n = muonsH.isValid() ? (*muonsH).size() : 0;
    ...
    if ((tracksH.isValid() && muonsH.isValid()) || !(this->skipNonExistingSrc_)) {
