// -*- C++ -*-
//
// Package:    DQMServices/ScoutingMonitor
// Class:      ScoutingTrackDzDebugger
//
// Description: EDAnalyzer that dumps a TTree with track/vertex quantities
//              for debugging the dz pull shape in ScoutingTrackMonitor.
//
// Usage: add to your cmsRun config, open output ROOT file, inspect the
//        "dzDebug/trackTree" TTree.
...
class ScoutingTrackDzDebugger : public edm::one::EDAnalyzer<edm::one::SharedResources> {
public:
  explicit ScoutingTrackDzDebugger(const edm::ParameterSet&);
  ~ScoutingTrackDzDebugger() override = default;
...
private:
  ...
  std::pair<unsigned int, const Run3ScoutingVertex*> findClosestVertex(
      const reco::Track&, const std::vector<Run3ScoutingVertex>&) const;
...
// ---------------------------------------------------------------------------
ScoutingTrackDzDebugger::ScoutingTrackDzDebugger(const edm::ParameterSet& iConfig)
    : tracksToken_{consumes<std::vector<Run3ScoutingTrack>>(
          iConfig.getParameter<edm::InputTag>("tracks"))},
      verticesToken_{consumes<std::vector<Run3ScoutingVertex>>(
          iConfig.getParameter<edm::InputTag>("vertices"))},
      beamSpotToken_{consumes<reco::BeamSpot>(
          iConfig.getParameter<edm::InputTag>("beamSpotLabel"))} {
  usesResource("TFileService");
}

// ---------------------------------------------------------------------------
void ScoutingTrackDzDebugger::beginJob() {
  edm::Service<TFileService> fs;
  tree_ = fs->make<TTree>("trackTree", "Scouting track dz debug tree");
...
// ---------------------------------------------------------------------------
void ScoutingTrackDzDebugger::analyze(const edm::Event& iEvent,
  ...
  for (const auto& trk : *tracksH) {
    ...
    b_dzErr_combined  = std::sqrt(b_dzErr_track  * b_dzErr_track  +
                                  b_vtx_zErr     * b_vtx_zErr);
    b_dxyErr_combined = std::sqrt(b_dxyErr_track * b_dxyErr_track +
                                  b_vtx_xErr     * b_vtx_xErr     +
                                  b_vtx_yErr     * b_vtx_yErr);

    // --- pulls (all three variants — compare these to find the right one) ---
    auto safeDivide = [](float num, float den) -> float {
      return (std::abs(den) > 1e-6f) ? num / den : -999.f;
    };
    ...
    b_vr_stored = std::sqrt(trk.tk_vx() * trk.tk_vx() +
                            trk.tk_vy() * trk.tk_vy());
...
// ---------------------------------------------------------------------------
reco::Track ScoutingTrackDzDebugger::makeRecoTrack(const Run3ScoutingTrack& sTrack) const {
...
// ---------------------------------------------------------------------------
reco::Vertex ScoutingTrackDzDebugger::makeRecoVertex(const Run3ScoutingVertex& sVertex) const {
...
// ---------------------------------------------------------------------------
std::pair<unsigned int, const Run3ScoutingVertex*>
ScoutingTrackDzDebugger::findClosestVertex(const reco::Track& track,
                                           const std::vector<Run3ScoutingVertex>& vertices) const {
...
// ---------------------------------------------------------------------------
void ScoutingTrackDzDebugger::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
#include "FWCore/Framework/interface/MakerMacros.h"
DEFINE_FWK_MODULE(ScoutingTrackDzDebugger);
