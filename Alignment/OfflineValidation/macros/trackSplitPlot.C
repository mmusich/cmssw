...
TCanvas *trackSplitPlot(Int_t nFiles,
  ...
  std::vector<bool> used(
      n);  //a file is not "used" if it's MC data and the x variable is run number, or if the filename is blank
  ...
  for (Int_t i = 0; i < n; i++) {
    ...
    used[i] = true;
    if ((xvar == "runNumber" ? findMax(files[i], "runNumber", 'x') < 2 : false) ||
        files[i] == "")  //if it's MC data (run 1), the run number is meaningless
    {
      used[i] = false;
    ...
    Double_t x = 0, y = 0, rel = 1, sigma1 = 1,
             sigma2 = 1,  //if !pull, we want to divide by sqrt(2) because we want the error from 1 track
        sigmaorg = 0;
  ...
  if (type == ScatterPlot)
    ...
    if (!allthesame && xvar != "runNumber")
      for (Int_t i = 0; i < n; i++) {
        p[i]->Scale(
            1.0 / lengths[i]);  //This does NOT include events that are out of the run number range (minrun and maxrun).
