...
MuonAnalyzer::MuonAnalyzer(const edm::ParameterSet& iConfig)
  ...
  // Create histograms
  hIsGlobalMuon =
      book<TH2F>("hIsGlobalMuon", "IsGlobalMuon vs Muon #eta;Muon #eta;IsGlobalMuon", 100, -2.8, 2.8, 2, -0.5, 1.5);
  hIsTrackerMuon =
      book<TH2F>("hIsTrackerMuon", "IsTrackerMuon vs Muon #eta;Muon #eta;IsTrackerMuon", 100, -2.8, 2.8, 2, -0.5, 1.5);
  hNumberOfMatches = book<TH2F>(
      "hNumberOfMatches", "NumberOfMatches vs Muon #eta;Muon #eta;NumberOfMatches", 100, -2.8, 2.8, 10, -0.5, 9.5);
  hNumberOfValidMuonHits = book<TH2F>("hNumberOfValidMuonHits",
                                      "NumberOfValidMuonHits vs Muon #eta;Muon #eta;NumberOfValidMuonHits",
                                      100,
                                      -2.8,
                                      2.8,
                                      50,
                                      -0.5,
                                      49.5);
  hNormalizedChi2 = book<TH2F>(
      "hNormalizedChi2", "Normalized #chi^{2} vs Muon #eta;Muon #eta;Normalized #chi^{2}", 100, -2.8, 2.8, 100, 0, 10);

  hIsoSumPtOverPt = book<TH2F>(
      "hIsoSumPtOverPt", "Isolation SumPt/Pt vs Muon #phi;Muon #phi;Isolation SumPt/Pt", 100, -M_PI, M_PI, 100, 0, 0.2);
  hIsoEmEtOverPt = book<TH2F>(
      "hIsoEmEtOverPt", "Isolation EmEt/Pt vs Muon #phi;Muon #phi;Isolation EmEt/Pt", 100, -M_PI, M_PI, 100, 0, 0.2);
  hIsoHadEtOverPt = book<TH2F>(
      "hIsoHadEtOverPt", "Isolation HadEt/Pt vs Muon #phi;Muon #phi;Isolation HadEt/Pt", 100, -M_PI, M_PI, 100, 0, 0.2);
  ...
  relIsoVsEtaPhi = book<TProfile2D>("RelIsoVsEtaPhi",
                                    "Relvative Isolation vs muon #eta/#phi;Muon #eta;Muon #phi;relative Isolation",
                                    100,
                                    -2.8,
                                    2.8,
                                    100,
...
void MuonAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (const auto& muon : *muons) {
    ...
    relIsoVsEtaPhi->Fill(
        muon.eta(),
        muon.phi(),
        (muon.isolationR03().sumPt + muon.isolationR03().emEt + muon.isolationR03().hadEt) / muon.pt());
  }
}
