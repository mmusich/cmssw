...
class GeneralTrackAnalyzerHisto_v3 : public edm::EDAnalyzer {
 public:
  GeneralTrackAnalyzerHisto_v3(const edm::ParameterSet& pset) {
  ...
  virtual void beginJob() {
    ...
    hBPixResXPrime       = fs->make<TH1D>("h_BPixResXPrime","BPix track X-residuals;res_{X'} [#mum];hits",100,-2000.,2000.);
    hFPixResXPrime       = fs->make<TH1D>("h_FPixResXPrime","FPix track X-residuals;res_{X'} [#mum];hits",100,-2000.,2000.);
    hFPixZPlusResXPrime  = fs->make<TH1D>("h_FPixZPlusResXPrime","FPix (Z+) track X-residuals;res_{X'} [#mum];hits",100,-2000.,2000.);
    hFPixZMinusResXPrime = fs->make<TH1D>("h_FPixZMinusResXPrime","FPix (Z-) track X-residuals;res_{X'} [#mum];hits",100,-2000.,2000.);
    					   			
    hBPixResYPrime       = fs->make<TH1D>("h_BPixResYPrime","BPix track Y-residuals;res_{Y'} [#mum];hits",100,-2000.,2000.);     
    hFPixResYPrime       = fs->make<TH1D>("h_FPixResYPrime","FPix track Y-residuals;res_{Y'} [#mum];hits",100,-2000.,2000.);     
    hFPixZPlusResYPrime  = fs->make<TH1D>("h_FPixZPlusResYPrime","FPix (Z+) track Y-residuals;res_{Y'} [#mum];hits",100,-2000.,2000.);
    hFPixZMinusResYPrime = fs->make<TH1D>("h_FPixZMinusResYPrime","FPix (Z-) track Y-residuals;res_{Y'} [#mum];hits",100,-2000.,2000.);
  ...
  void fillByIndex(std::map<unsigned int, TH1D*>& h, unsigned int index, double x)
  {
	...
	infoVector.push_back(token);
      ...
      throw edm::Exception(edm::errors::LogicError) << " GeneralTrackAnalyzer::fillByIndex(): the packed toplogy index " << index << " is invalid. \n " 
...
DEFINE_FWK_MODULE(GeneralTrackAnalyzerHisto_v3);
