...
EopElecTreeWriter::EopElecTreeWriter(const edm::ParameterSet& iConfig) {
  ...
  theEndCapSupClusCollectionToken = consumes<reco::SuperClusterCollection>(edm::InputTag("multi5x5SuperClusters","multi5x5EndcapSuperClusters"));
...
void EopElecTreeWriter::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
 ...
 Double_t EnergyHcalIn01; Double_t EnergyHcalIn02; Double_t EnergyHcalIn03;
 Double_t EnergyHcalIn04; Double_t EnergyHcalIn05;
 Double_t etaWidth; Double_t phiWidth;
 Int_t algo_ID; Double_t EnergyEcal; Double_t fbrem;
 Double_t pin; Double_t etaIn; Double_t phiIn;
 Double_t pout; Double_t etaOut; Double_t phiOut;
 Double_t MaxPtIn01; Double_t SumPtIn01; Bool_t NoTrackIn0015;
 Double_t MaxPtIn02; Double_t SumPtIn02; Bool_t NoTrackIn0020;
 Double_t MaxPtIn03; Double_t SumPtIn03; Bool_t NoTrackIn0025;
 Double_t MaxPtIn04; Double_t SumPtIn04; Bool_t NoTrackIn0030;
 Double_t MaxPtIn05; Double_t SumPtIn05; Bool_t NoTrackIn0035; Bool_t NoTrackIn0040;
 Double_t dRSC_first; Double_t dRSC_second; Double_t etaSC;
 Double_t phiSC;
 ...
 MultiTrajectoryStateTransform mtsTransform(trackerGeom_.product(),
					    magField_.product());
 ...
 if(vertex->size()==0){cout<<"Error: no primary vertex found!"<<endl; return;}
 reco::Vertex vert; 
 vert = vertex->front();
 ...
 if(subGeo==NULL)cout<<"ERROR: unable to find SubDetector geometry!!!"<<std::endl;
 ...
 if(rhc==NULL)cout<<"ERROR !!!"<<std::endl;
 ...
 for (unsigned int i=0;i<triggerNames_.size();i++){
   if (triggerNames_[i].find(pattern)!=0) continue; 
   ...
   if (myTrigger.index==-1) continue;
   myTrigger.fired    =  trigRes->wasrun(myTrigger.index) && trigRes->accept(myTrigger.index) && !trigRes->error(myTrigger.index);
 ...
 for ( std::map<std::string, EopTriggerType>::const_iterator it=HLTpaths.begin();it!=HLTpaths.end();it++){
   if (it->second.fired) {TrigTag=true; firstFiredPath=it->first; break; }
 }
 if (!TrigTag) return;
 ...
 */   
 ...
 if ( testindex >= triggerEvent->sizeFilters() ) return;
 ...
 if (HLTelectrons.empty()) return;
  ...
  for (unsigned int i=0;i<tracks->size();i++){
    nTracks->Fill(0.5);
    double deltar = reco::deltaR((*tracks)[i].eta(), 
				 (*tracks)[i].phi(), 
				 HLTelectrons[HighPtIndex]->eta(), 
				 HLTelectrons[HighPtIndex]->phi());
  ...
  if(nRejected==0) return;
  nEventsHLTrejected->Fill(0.5);

  if (filterTracks.empty()) return;
  ...
  for(std::vector<reco::GsfElectronCore>::const_iterator elec = electrons->begin();elec!=electrons->end();elec++){
      ...
      if((elec->gsfTrack())->pt()<10.) continue;
      
      double DELTAR=0;
      DELTAR=reco::deltaR(
			  (elec->superCluster())->eta(),
			  (elec->superCluster())->phi(),
			  etaGSF,
			  (elec->gsfTrack())->phi()
			  );
      
      if (DELTAR<0.1)distToClosestSCgsf->Fill(DELTAR);
  ...
  for(std::vector<const reco::GsfTrack*>::const_iterator itrack = filterTracks.begin();itrack!=filterTracks.end();++itrack){

    const reco::GsfTrack* track = (*itrack);

     // initializing variables
    isEcalDriven=false; isTrackerDriven=false; isBarrel=false; isEndcap=false;
    etaWidth=0; phiWidth=0; etaSC=0; phiSC=0; fbrem=0;
    pin=0; etaIn=0; phiIn=0; pout=0; etaOut=0; phiOut=0;
    algo_ID=0; EnergyEcal=0; dRSC_first =999; dRSC_second =9999;
    nbSC=0; nBasicClus=0; 
    ...
    EnergyHcalIn01=0; EnergyHcalIn02=0; EnergyHcalIn03=0; 
    EnergyHcalIn04=0; EnergyHcalIn05=0;
    ...
    for (std::vector<HBHERecHit>::const_iterator hcal=(*HcalHits).begin(); hcal!=(*HcalHits).end(); hcal++){
      ...
      if(dR < 0.1) EnergyHcalIn01+=hcal->energy();
      if(dR < 0.2) EnergyHcalIn02+=hcal->energy();
      if(dR < 0.3) EnergyHcalIn03+=hcal->energy();
      if(dR < 0.4) EnergyHcalIn04+=hcal->energy();
      if(dR < 0.5) EnergyHcalIn05+=hcal->energy();
    }
    ...
    for(std::vector<const reco::GsfTrack*>::const_iterator itrack1 = filterTracks.begin();itrack1!=filterTracks.end();++itrack1){
      const reco::GsfTrack* track1 = (*itrack1);

      if (track == track1) continue;

      double etaIn1=0.; double phiIn1=0.;
	...
	phiIn1 = inMom1.phi();
      ...
      if (etaIn1==0 && phiIn1==0) continue;  
      
      double dR= reco::deltaR( etaIn1, phiIn1, etaIn, phiIn ) ;
      
      // different radius of inner isolation cone
      if(dR <  0.015)NoTrackIn0015=false;
      if(dR <  0.020)NoTrackIn0020=false;
      if(dR <  0.025)NoTrackIn0025=false;
      if(dR <  0.030)NoTrackIn0030=false;
      if(dR <  0.035)NoTrackIn0035=false;
      if(dR <  0.040)NoTrackIn0040=false;
      
      //calculate maximum Pt and sum Pt inside cones of different radius
      if( dR <  0.1 ) {
	SumPtIn01 += track1->pt();
	if (track1->pt()>MaxPtIn01) { MaxPtIn01 = track1->pt(); }
      }
      
      if( dR <  0.2 ) {
	SumPtIn02 += track1->pt();
	if (track1->pt()>MaxPtIn02) { MaxPtIn02 = track1->pt(); }
      }
      if( dR <  0.3 ) {
	SumPtIn03 += track1->pt();
	if (track1->pt()>MaxPtIn03) { MaxPtIn03 = track1->pt(); }
      }
      if( dR <  0.4 ) {
	SumPtIn04 += track1->pt();
	if (track1->pt()>MaxPtIn04) { MaxPtIn04 = track1->pt(); }
      }
      if( dR <  0.5 ) {
	SumPtIn05 += track1->pt();
	if (track1->pt()>MaxPtIn05) { MaxPtIn05 = track1->pt(); }
      }
    }
    ...
    if(track->eta() < 1.55 || track->eta() > -1.55){
      for (std::vector<reco::SuperCluster>::const_iterator SC=BarrelSupClusCollection->begin(); SC!=BarrelSupClusCollection->end(); SC++){
	dRSC= 0; 
	dRSC= reco::deltaR( SC->eta(), SC->phi(), etaAtEcal, phiIn);

	if(dRSC<dRSC_first){dRSC_first=dRSC;} // distance in eta-phi plan to closest SC
	else if(dRSC<dRSC_second){dRSC_second=dRSC;} // to next closest SC
    ...
    if(track->eta() < -1.44 || track->eta() > 1.44){
      for (std::vector<reco::SuperCluster>::const_iterator SC=EndcapSupClusCollection->begin(); SC!=EndcapSupClusCollection->end(); SC++){
	dRSC= 0; 
	dRSC= reco::deltaR( SC->eta(), SC->phi(), etaAtEcal, phiIn);
    ...
    if (dRSC_first < 0.1) distToClosestSC->Fill(dRSC_first);
    if (nbSC==1)counter1->Fill(0.5);
    if (nbSC==0)counter2->Fill(0.5);
    
    if(nbSC>1 || nbSC==0) continue;
    cut_OneSCmatch->Fill(0.5);
    
    if (isBarrel && isEndcap){cout<<"Error: Super Cluster double matching!"<<endl; return;}

    EcalEnergy->Fill(EnergyEcal);
    HcalVSEcal->Fill(EnergyEcal, EnergyHcalIn02);

    // E over p plots
    if(track->charge()<0) Eop_InnerNegative->Fill(EnergyEcal/pin);
    if(track->charge()>0) Eop_InnerPositive->Fill(EnergyEcal/pin);
    
    //Check if track-SuperCluster matching is Ecal or Tracker driven
    edm::RefToBase<TrajectorySeed> seed = track->extra()->seedRef() ;
    if (seed.isNull()){ edm::LogError("GsfElectronCore")<<"The GsfTrack has no seed ?!" ; }
    else{
      reco::ElectronSeedRef elseed = seed.castTo<reco::ElectronSeedRef>() ;
      if (elseed.isNull()){ edm::LogError("GsfElectronCore")<<"The GsfTrack seed is not an ElectronSeed ?!" ; }
      else{
...
// ------------ method called once each job just before starting event loop  ------------
void 
EopElecTreeWriter::beginRun(const edm::Run& iRun, const edm::EventSetup& iSetup){
  bool changed=true;
