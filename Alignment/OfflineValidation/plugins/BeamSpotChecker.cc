...
namespace bscheck{
  ...
  struct Significance{
    ...
    float getSig(bool verbose){
      ...
      if(verbose){
	edm::LogInfo("BeamSpotChecker") << "A= " << m_A << "+/-" << m_ErrA << "    " << "B= " << m_B << "+/-" << m_ErrB << " | delta="  << std::abs(m_A - m_B) << "+/-" << std::sqrt((m_ErrA*m_ErrA)+(m_ErrB*m_ErrB)) << std::endl; 
      }
  ...
  bsFromDBToken_(consumes<reco::BeamSpot>(iConfig.getParameter<edm::InputTag>("bsFromDB")))
{
  ...
  if(warningThreshold_>throwingThreshold_){
    throw cms::Exception("ConfigurationError")<< __PRETTY_FUNCTION__ << "\n Warning threshold ("<< warningThreshold_ << ") cannot be smaller than the throwing threshold (" << throwingThreshold_ << ")"<< std::endl; 
  }
}
...
void
BeamSpotChecker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  ...
  std::string msg = 
    " |delta X0|=" + std::to_string(std::abs(deltaX0)* cmToum)
  ...
  for(unsigned int i=0;i<3;i++){
    ...
    if(sig > throwingThreshold_){
      ...
      throw cms::Exception("BeamSpotChecker") << "[" << __PRETTY_FUNCTION__  << "] \n DB-Event BeamSpot "<< labels.at(i)  <<" distance sigificance " << sig << ", exceeds the threshold of "<< throwingThreshold_ << "!" << std::endl;
    }
    else if(sig > warningThreshold_){
      edm::LogWarning("BeamSpotChecker") << msg.c_str() << std::endl;
      edm::LogWarning("BeamSpotChecker") << "[" << __PRETTY_FUNCTION__ << "]  \n  DB-Event BeamSpot " << labels.at(i) << " distance significance " << sig << ", exceeds the threshold of "<< warningThreshold_ << "!" << std::endl;
    }
  //i++;
  }
...
std::array<float,6>
BeamSpotChecker::compareBS(const reco::BeamSpot& BSA,const reco::BeamSpot& BSB){
  ...
  std::array<float,6> ret = {{xS.getSig(false),yS.getSig(false),zS.getSig(false),
			      xWS.getSig(false),yWS.getSig(false),zWS.getSig(false)}};
  return ret;
}
...
void
BeamSpotChecker::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void
BeamSpotChecker::endJob()
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
BeamSpotChecker::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
