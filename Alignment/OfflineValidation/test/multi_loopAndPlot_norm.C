...
Int_t def_markers[9] = {kFullCross,
                        kFullSquare,
                        kFullCircle,
                        kOpenSquare,
                        kOpenCircle,
  ...
  while ((key = (TKey *)nextkey())) {
    ...
    if (obj->IsA()->InheritsFrom("TH1")) {
      if (obj->IsA()->InheritsFrom("TH2"))
        continue;
      if ((TString(obj->GetName())).Contains("_per_LumiBlock"))
        continue;
...
void plotHistograms(std::vector<TH1 *> histos, const std::vector<TString> &labels, bool isNormalized) {
  ...
  for (unsigned int i = 0; i < histos.size(); i++) {
    TString mean =
        histos[i]->GetMean() < 1000 ? Form("%.1f", histos[i]->GetMean()) : Form("%.1fk", histos[i]->GetMean() / 1000.);
    TString rms =
        histos[i]->GetRMS() < 1000 ? Form("%.1f", histos[i]->GetRMS()) : Form("%.1fk", histos[i]->GetRMS() / 1000.);
