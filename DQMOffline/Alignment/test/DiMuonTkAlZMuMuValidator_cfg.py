...
process.seqRefitting = cms.Sequence(process.offlineBeamSpot + process.refittedMuons + process.refittedVtxTracks + process.offlinePrimaryVerticesFromRefittedTrks)
...
if (options.resonance == 'Z'):
    ...
    # change input collection because of refit
    process.ALCARECOTkAlDiMuonAndVertexVtxDQM.muonTracks = 'refittedMuons'
    process.ALCARECOTkAlDiMuonAndVertexVtxDQM.vertices = 'offlinePrimaryVerticesFromRefittedTrks'
    process.ALCARECOTkAlDiMuonMassBiasDQM.muonTracks = 'refittedMuons'
...
process.DQMoutput = cms.OutputModule("DQMRootOutputModule",
    ...
    fileName = cms.untracked.string('file:step3_inDQM_'+options.resonance+'_'+options.scenario+'.root'),
...
connection_string = "sqlite_file:/eos/cms/store/group/alca_trackeralign/mteroerd/output_baseline/payloads_HG.db" if (options.scenario == 'baseline') else "sqlite_file:/eos/cms/store/group/alca_trackeralign/mteroerd/output_g4refitter/payloads_HG.db"
process.GlobalTag.toGet = cms.VPSet(cms.PSet(record = cms.string('TrackerAlignmentRcd'),
                                             tag = cms.string("SiPixelAliHG_pcl"),                                            
                                             connect = cms.string(connection_string)))

process.refitting_step =  cms.Path(process.seqRefitting)
process.dqmoffline_step = cms.EndPath(process.seqALCARECOTkAlDQM)
process.DQMoutput_step = cms.EndPath(process.DQMoutput)

process.schedule = cms.Schedule(process.refitting_step,process.dqmoffline_step,process.DQMoutput_step)
