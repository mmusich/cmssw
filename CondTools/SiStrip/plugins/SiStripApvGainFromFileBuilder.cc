...
SiStripApvGainFromFileBuilder::SiStripApvGainFromFileBuilder(const edm::ParameterSet& iConfig)
    : gfp_(iConfig.getUntrackedParameter<edm::FileInPath>(
          "geoFile", edm::FileInPath("CalibTracker/SiStripCommon/data/SiStripDetInfo.dat"))),
...
void SiStripApvGainFromFileBuilder::analyze(const edm::Event& evt, const edm::EventSetup& iSetup) {
  ...
  edm::LogInfo("Workflow") << "@SUB=analyze"
  ...
  for (std::map<uint32_t, SiStripDetInfoFileReader::DetInfo>::const_iterator it = DetInfos.begin();
    ...
    for (unsigned short j = 0; j < it->second.nApvs; j++) {
      ...
      try {
        ...
        LogTrace("Debug") << it->first << "  " << std::setw(3) << j << "   " << std::setw(3) << connection.size()
                          << "   " << std::setw(3) << summary.onlineAPV_id << "    " << std::setw(3) << summary.FED_id
                          << "   " << std::setw(3) << summary.FED_ch << "   " << std::setw(3) << summary.i2cAdd << "   "
...
void SiStripApvGainFromFileBuilder::read_tickmark() {
  ...
  if (!thickmark_heights.is_open()) {
    edm::LogError("FileNotFound") << "@SUB=read_ticlmark"
                                  << "File with thickmark height file " << filename << " cannot be opened!"
                                  << std::endl;
    return;
  }
  ...
  for (;;) {
    ...
    if (!(thickmark_heights.eof() || thickmark_heights.fail())) {
      ...
      edm::LogInfo("Workflow") << "@SUB=read_tickmark"
                               << "EOF of " << filename << " reached." << std::endl;
      break;
    } else if (thickmark_heights.fail()) {
      edm::LogError("FileiReadError") << "@SUB=read_tickmark"
                                      << "error while reading " << filename << std::endl;
      break;
    }
  }
