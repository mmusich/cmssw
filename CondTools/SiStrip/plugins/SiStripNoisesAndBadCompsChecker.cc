  ...
  for (const auto& it : DetInfos) {
    ...
    for (int strip = 0; strip < sistrip::STRIPS_PER_APV * nAPVs; ++strip) {
      ...
      if (!badChannels[strip]) {
        //const auto& theNoise = noise.getNoise(strip, detNoiseRange);
        if (!noise.isInRange(strip, detNoiseRange)) {
          if (count < printdebug_) {
            edm::LogPrint("SiStripNoisesAndBadCompsChecker")
    ...
    if (!obj.put(it.first, theSiStripVector))
      edm::LogError("SiStripNoisesBuilder") << "[SiStripNoisesBuilder::analyze] detid already exists";
  }  // loop on the detids
  edm::LogPrint("SiStripNoisesAndBadCompsChecker") << "Found " << count << " strips in out-of-bounds!";

  if (writePayload_) {
    edm::LogInfo("SiStripNoisesAndBadCompsChecker") << "Will write an updated fixed payload";
