...
// Class:      SiStripChannelGainTrendPlotter
//
/**\class SiStripChannelGainTrendPlotter SiStripChannelGainTrendPlotter.cc CondTools/SiStrip/plugins/SiStripChannelGainTrendPlotter.cc
...
class SiStripChannelGainTrendPlotter : public edm::one::EDAnalyzer<edm::one::SharedResources> {
public:
  explicit SiStripChannelGainTrendPlotter(const edm::ParameterSet&);
  ~SiStripChannelGainTrendPlotter();
...
//
SiStripChannelGainTrendPlotter::SiStripChannelGainTrendPlotter(const edm::ParameterSet& iConfig) {
...
SiStripChannelGainTrendPlotter::~SiStripChannelGainTrendPlotter() {
...
void SiStripChannelGainTrendPlotter::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
  ...
  if (hasG1IOV || hasG2IOV) {
    ...
    tkhisto = new TkHistoMap(tkDetMapHandle.product(), "G1G2", Form("G2G2_%i", run), -1.);  //here
    ...
    for (size_t id = 0; id < detid.size(); id++) {
      ...
      for (int it = 0; it < rangeG1.second - rangeG1.first; it++) {
        ...
        if (DEBUG) {
          ...
          std::cout << "detid " << detid[id] << " \t " << apv++ << " \t  G1: " << G1 << " \t  G2: " << G2
                    << " \t G1*G2: " << G1G2 << std::endl;
        }
      ...
      g1g2dump << std::endl;
      if (tkhisto != nullptr)
        tkhisto->fill(detid[id], sumOfGains / nAPV);
...
void SiStripChannelGainTrendPlotter::beginJob() {
  ...
  for (int i = partitions::TIB; i != partitions::NUM_OF_TYPES; i++) {
    ...
    for (int j = 0; j < nIOVs_; ++j) {
      h_gainByPart_[i][j] =
          dirs[i].make<TH1D>(Form("G_%s_IOV%i", this->getStringFromEnum(part).c_str(), j),
                             Form("%s Gain for IOV %i; Gain; n. APV", this->getStringFromEnum(part).c_str(), j),
                             nGainBins_,
                             0.,
                             2.);
    }
...
void SiStripChannelGainTrendPlotter::endJob() {
  ...
  for (std::map<int, std::map<partitions::layers, TH1D*> >::iterator it = gainTrendByPartition_.begin();
       it != gainTrendByPartition_.end();
       ++it) {
  ...
  for (unsigned int i = 0; i < theBoundaries_.size(); i++) {
    ...
    output << "|====================================== " << std::endl << "| run: " << the_r << std::endl;
    ...
    for (int j = partitions::TIB; j != partitions::NUM_OF_TYPES; j++) {
      ...
      theMap[part]->SetTitle(Form("%s Gain for IOV %i (%.0f,%.0f)",
                                  this->getStringFromEnum(part).c_str(),
                                  i,
                                  theBoundaries_[i],
                                  theBoundaries_[i + 1]));
      ...
      output << "| " << this->getStringFromEnum(part) << " <G>: " << std::setw(4) << the_gain_averages_[part][i]
             << " RMS(G): " << std::setw(4) << the_gain_RMS_[part][i] << std::endl;
    }
  }
  ...
  for (unsigned int i = 0; i < theBoundaries_.size(); i++) {
    ...
    std::cout << "i: " << i << "| midPoints[" << i << "]:" << std::setw(8) << midPoints[i] << " binWidths[" << i
              << "]:" << std::setw(8) << binWidths[i] << " left: " << std::setw(8) << midPoints[i] - binWidths[i]
              << " right: " << std::setw(8) << midPoints[i] + binWidths[i] << std::endl;
  }
  ...
  for (int i = partitions::TIB; i != partitions::NUM_OF_TYPES; i++) {
    ...
    g_GainsByPartition_ByIOV[part] = fs->make<TGraphErrors>(theBoundaries_.size(),
                                                            &(midPoints[0]),
                                                            &(the_gain_averages_[part][0]),
                                                            &(binWidths[0]),
                                                            &(ex_[0]));  //&(the_gain_RMS_[part][0]));
    ...
    g_GainsRMSByPartition_ByIOV[part] =
        fs->make<TGraph>(theBoundaries_.size(), &(theBoundaries_[0]), &(the_gain_RMS_[part][0]));
    ...
    h_GainsByPartition_ByIOV[part] = fs->make<TH1F>(
        Form("h_average_Gain_%s", this->getStringFromEnum(part).c_str()),
        Form("average Gain G1*G2 %s; IOV (run number); #LT G1*G2 #GT", this->getStringFromEnum(part).c_str()),
        nPoints - 1,
        &(binStops_[0]));

    h_GainsRMSByPartition_ByIOV[part] =
        fs->make<TH1F>(Form("h_RMS_Gain_%s", this->getStringFromEnum(part).c_str()),
    ...
    h2_GainsByPartition_ByIOV[part] =
        fs->make<TH2F>(Form("h2_Gain_%s", this->getStringFromEnum(part).c_str()),
    ...
    for (unsigned int j = 0; j < nPoints - 1; j++) {
      ...
      for (int k = 0; k < theMap[part]->GetNbinsX(); k++) {
        ...
        h2_GainsByPartition_ByIOV[part]->Fill(
            theBoundaries_[j], theMap[part]->GetBinCenter(k + 1), theMap[part]->GetBinContent(k + 1));
...
std::pair<int, std::pair<int, int> > SiStripChannelGainTrendPlotter::typeAndLayerFromDetId(
    const DetId& detId, const TrackerTopology* tTopo) const {
  int layerNumber = 0;
  int sideNumber = 0;
...
// -------------- method to get the topology from the detID ------------------------------
std::string SiStripChannelGainTrendPlotter::getStringFromEnum(partitions::layers e) {
  switch (e) {
    case partitions::TIB:
      return "TIB";
...
void SiStripChannelGainTrendPlotter::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(SiStripChannelGainTrendPlotter);
