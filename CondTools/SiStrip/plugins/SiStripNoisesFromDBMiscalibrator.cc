...
SiStripNoisesFromDBMiscalibrator::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   for (const auto & d : detid) {
	 ...
	 noise*=params.m_scaleFactor;
       ...
       if(params.m_doSmear){
	 float smearedGain = random->Gaus(noise,params.m_smearFactor);
	 noise=smearedGain;
       }
     ...
     //std::cout<<d<<" " << nStrips << std::endl;

   } // loop over DetIds

   std::unique_ptr<SiStripNoises> theSiStripNoises = this->getNewObject(theMap);
...
// 	//edm::LogInfo("SiStripNoisesFromDBMiscalibrator") << "detid " << it->first << " \t"
// 	//						   << " strip " << t_strip << " \t"
// 	//						   << " not found" << std::endl; 	    
	
// 	//std::cout << "detid " << it->first << " \t" << " strip " << t_strip << " \t" << " not found" << std::endl; 	   
...
SiStripNoisesFromDBMiscalibrator::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.addUntracked<std::string>("record","SiStripSiStripNoiseRcd");
