...
from FWCore.ParameterSet.MassReplace import massReplaceESInputTag,massReplaceParameter

def replacePixelCPETemplateWithGenericESInpuTag(process):    
    massReplaceESInputTag(process,old="WithAngleAndTemplate",new="WithTrackAngle",verbose=True)
    process.lowPtGsfElectronSeedsTmp.TTRHBuilder = cms.ESInputTag("","WithTrackAngle")
    
    return process

def replacePixelCPETemplateWithGenericParameter(process):    
    massReplaceParameter(process,name="TTRHBuilder",old="WithAngleAndTemplate",new="WithTrackAngle",verbose=True)
    massReplaceParameter(process,name="TTRHBuilder",old="WithAngleAndTemplateWithoutProbQ",new="WithTrackAngle",verbose=True)
    massReplaceParameter(process,name="TrackerRecHitBuilder",old="WithAngleAndTemplate",new="WithTrackAngle",verbose=True)
    massReplaceParameter(process,name="ttrhBuilderName",old="WithAngleAndTemplate",new="WithTrackAngle",verbose=True)
    
    return process

def replacePixelCPETemplateWithGeneric(process):
    replacePixelCPETemplateWithGenericESInpuTag(process)
    replacePixelCPETemplateWithGenericParameter(process)
    return process
