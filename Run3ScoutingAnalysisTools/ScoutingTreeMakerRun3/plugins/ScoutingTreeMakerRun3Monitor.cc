...
void ScoutingTreeMakerRun3Monitor::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  // ------------------------------------------------
  if (!(idx.size() == 2)) { /*cout<<"failed offline muons"<<endl;*/
    return;
  }
  ...
  if (checkOSCharge > 0) { /*cout<<"not OS pair"<<endl;*/
    return;
  }
  ...
  // -----------------------------------------------------
  if (!passDST) { /*cout<<"failed DST"<<endl;*/
    return;
  }
  ...
  // -------------------------------------------------
  if (!(idx_scout.size() == 2)) { /*cout<<"failed scouting muons"<<endl;*/
    return;
  }
  ...
  if (checkOSCharge_scout > 0) { /*cout<<"not OS pair"<<endl;*/
    return;
  }
  ...
  // -----------------------------------------------------------------------------------
  pfIso1 = (offlineMuonsH->at(idx[0]).chargedHadronIso() +
            std::max(offlineMuonsH->at(idx[0]).photonIso() + offlineMuonsH->at(idx[0]).neutralHadronIso() -
                         0.5 * offlineMuonsH->at(idx[0]).puChargedHadronIso(),
                     0.0)) /
           offlineMuonsH->at(idx[0]).pt();
  pfIso2 = (offlineMuonsH->at(idx[1]).chargedHadronIso() +
            std::max(offlineMuonsH->at(idx[1]).photonIso() + offlineMuonsH->at(idx[1]).neutralHadronIso() -
                         0.5 * offlineMuonsH->at(idx[1]).puChargedHadronIso(),
                     0.0)) /
           offlineMuonsH->at(idx[1]).pt();
  ...
  */
  for (int mu_dr1 = 0; mu_dr1 < 2; mu_dr1++) {
    ...
    */
    dr1 = sqrt((offlineMuonsH->at(idx[0]).eta() - muonsH->at(idx_scout[mu_dr1]).eta()) *
                   (offlineMuonsH->at(idx[0]).eta() - muonsH->at(idx_scout[mu_dr1]).eta()) +
               (offlineMuonsH->at(idx[0]).phi() - muonsH->at(idx_scout[mu_dr1]).phi()) *
                   (offlineMuonsH->at(idx[0]).phi() - muonsH->at(idx_scout[mu_dr1]).phi()));
    //cout<<"dR = "<<dr1<<endl;
  ...
  for (int mu_dr2 = 0; mu_dr2 < 2; mu_dr2++) {
    ...
    */
    dr2 = sqrt((offlineMuonsH->at(idx[1]).eta() - muonsH->at(idx_scout[mu_dr2]).eta()) *
                   (offlineMuonsH->at(idx[1]).eta() - muonsH->at(idx_scout[mu_dr2]).eta()) +
               (offlineMuonsH->at(idx[1]).phi() - muonsH->at(idx_scout[mu_dr2]).phi()) *
                   (offlineMuonsH->at(idx[1]).phi() - muonsH->at(idx_scout[mu_dr2]).phi()));
    //cout<<"dR = "<<dr2<<endl;
  ...
  */
  ...
  if (idx1_scout == idx2_scout) {
    if (dr_matching_1 < dr_matching_2) {
      idx1_scout = 0;
      idx2_scout = 1;
    } else if (dr_matching_1 > dr_matching_2) {
      idx1_scout = 1;
      idx2_scout = 0;
    }
  }
