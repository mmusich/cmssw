...
std::vector<TransientVertex> SinglePrimaryVertexFitter::fit(const std::vector<reco::TransientTrack>& tracks,
                                                            const std::vector<TransientVertex>& clusters,
                                                            const reco::BeamSpot& beamspot,
                                                            const bool useBeamConstraint) {
  std::cout << "SinglePrimaryVertexFitter::fit  clusters.size=" << clusters.size()
            << "  use beamspot = " << useBeamConstraint << std::endl;
