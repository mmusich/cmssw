...
TEST_CASE("SiStripBadStrip testing", "[SiStripBadStrip]") {
  ...
  SECTION("Check barrel plotting") {
    ...
    int counter{0};
    for(unsigned short fs=0;fs<=2047;fs++){
      for(unsigned short rng=0;rng<=2047;rng++){
    
      auto encoded = testObject.encodePhase2(fs,rng);  
      auto decoded = testObject.decodePhase2(encoded);

      if(counter<10){
	std::cout <<"input: (" << fs <<"," << rng <<") | encoded:" << std::setw(10) << encoded
		  <<"| decoded : (" << decoded.firstStrip <<"," << decoded.range << ")" << std::endl;
      }     
