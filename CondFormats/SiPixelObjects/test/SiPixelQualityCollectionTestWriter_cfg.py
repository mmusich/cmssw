...
##
process.CondDB.connect = 'sqlite_file:SiPixelStatusScenarios.db'
process.PoolDBOutputService = cms.Service("PoolDBOutputService",
                                          process.CondDB,
                                          timetype = cms.untracked.string('runnumber'),
                                          toPut = cms.VPSet(cms.PSet(record = cms.string('SiPixelStatusScenariosRcd'),
                                                                     tag = cms.string('SiPixeQualityCollection_StuckTBM_2018_v1_mc')
                                                                     )
                                                            )
                                          )

process.WriteInDB = cms.EDAnalyzer("SiPixelQualityCollectionTestWriter",
