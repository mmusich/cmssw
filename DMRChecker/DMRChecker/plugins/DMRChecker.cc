...
public:
  ...
  void updateOnlineMomenta(running::estimatorMap &myDetails,const uint32_t& theID,
			   const float& the_data,const float& the_pull,const running::dir& theDir);
...
DMRChecker::DMRChecker(const edm::ParameterSet& iConfig)
  : avalidator_(iConfig, consumesCollector()),
    offlinePrimaryVerticesToken_(consumes<reco::VertexCollection>(iConfig.getParameter<std::string>("VertexCollection"))){
...
//*************************************************************
void DMRChecker::updateOnlineMomenta(running::estimatorMap &myDetails,const uint32_t& theID,const float& the_data,const float& the_pull,const running::dir& theDir) {

  // if the detid has never occcurred yet, set the local orientations
  if (myDetails.find(theID) == myDetails.end()) {
    if(theDir==running::X) myDetails[theID].isX=true;
    if(theDir==running::Y) myDetails[theID].isY=false;
  }
