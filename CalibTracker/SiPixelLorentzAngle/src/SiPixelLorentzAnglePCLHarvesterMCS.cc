...
namespace SiPixelLAHarvestMCS {
  ...
  double MCSFitFunction(double* x, double* par) {
    double arg;
  ...
  void fcn_func(Int_t& npar, double* deriv, double& f, double* par, Int_t flag);
  ...
  public:
    ...
    friend void fcn_func(Int_t& npar, double* deriv, double& f, double* par, Int_t flag);
...
private:
  ...
  FpixMuHResults fpixMuHResults_{};
...
void SiPixelLorentzAnglePCLHarvesterMCS::beginRun(const edm::Run& iRun, const edm::EventSetup& iSetup) {
  ...
  tTopo_ = &iSetup.getData(topoEsTokenBR_);
  currentLorentzAngle_ = &iSetup.getData(siPixelLAEsToken_);
  PixelTopologyMap map = PixelTopologyMap(geom, tTopo_);
  ...
  if (!newmodulelist_.empty()) {
    for (auto const& modulename : newmodulelist_) {
      if (modulename.find("BPix_") != std::string::npos) {
        ...
        const auto& detId = bn.getDetId(tTopo_);
        ...
        const auto& detId = en.getDetId(tTopo_);
...
void SiPixelLorentzAnglePCLHarvesterMCS::dqmEndJob(DQMStore::IBooker& iBooker, DQMStore::IGetter& iGetter) {
  ...
  hists_.h_fpixIsMeanHistoFitValid_ =
      iBooker.book1D("fpixIsMeanHistoFitValid",
                     "is fit valid by rings/panels/sides; ring/panel/side; is fit valid",
                     nBinsMeanHistsFitSummary,
                     -0.5,
                     nBinsMeanHistsFitSummary - 0.5);
  hists_.h_fpixMinClusterSizeCotAngle_ =
      iBooker.book1D("fpixMinClusterSizeCotAngle",
                     "cot angle of minimal cluster size by rings/panels/sides/angles; ring/panel/side/angle; ",
                     nBinsMeanHistsFitSummary,
                     -0.5,
                     nBinsMeanHistsFitSummary - 0.5);
  ...
  for (int r = 0; r < hists_.nRings_; ++r) {
    for (int p = 0; p < hists_.nPanels_; ++p) {
      for (int s = 0; s < hists_.nSides_; ++s) {
        ...
        hists_.h_fpixMean_[idx] = iGetter.get(fmt::format("{}/R{}_P{}_z{}_alphaMean", dqmDir_, r + 1, p + 1, s + 1));
        hists_.h_fpixMean_[idxBeta] = iGetter.get(fmt::format("{}/R{}_P{}_z{}_betaMean", dqmDir_, r + 1, p + 1, s + 1));
        hists_.h_fpixAngleSize_[idx] = iGetter.get(fmt::format("{}/R{}_P{}_z{}_alpha", prefix_, r + 1, p + 1, s + 1));
  ...
  for (int r = 0; r < hists_.nRings_; ++r) {
    for (int p = 0; p < hists_.nPanels_; ++p) {
      ...
      fpixMuHResults_.insert(std::make_pair(fpixPartNames, std::make_pair(muH, muH)));
    }
  }
  ...
  for (const auto& [id, value] : currentLorentzAngle_->getLorentzAngles()) {
    ...
    if (ID.subdetId() == PixelSubdetector::PixelEndcap) {
      PixelEndcapName pen(ID, tTopo_, true);  // use det-id phaseq
      int panel = pen.pannelName();
      int ring = pen.ringName();
      std::string fpixPartNames = "R" + std::to_string(ring) + "_P" + std::to_string(panel);
      if (fpixMuHResults_.find(fpixPartNames) != fpixMuHResults_.end()) {
        double measuredMuH = fpixMuHResults_[fpixPartNames].first;
