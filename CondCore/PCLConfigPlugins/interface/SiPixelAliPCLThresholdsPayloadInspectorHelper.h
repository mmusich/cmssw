  ...
  *************************************************/
  ...
  public:
    ...
    bool fill() override {
      ...
      const auto& local_end_of_types =
          isHighGranularity_ ? PCLThresholdsPI::END_OF_TYPES : PCLThresholdsPI::FRACTION_CUT;
      ...
      for (const auto& alignable : alignables) {
        ...
        for (int foo = PayloadType::X; foo != PayloadType::extra_DOF; foo++) {
          ...
          for (int bar = PCLThresholdsPI::DELTA; bar != local_end_of_types; bar++) {
  ...
  public:
    ...
    bool fill() override {
      ...
      if (!isEqual(alignables, alignables2)) {
        edm::LogError(label_) << "Cannot compare the two AlignPCLThresholds objects, as the list of alignables differs";
        return false;
      }
      ...
      auto ThresholdsColor =
          std::make_unique<TH2F>("ThresholdsC", "", alignables.size(), 0, alignables.size(), N_Y_BINS, 0, N_Y_BINS);
