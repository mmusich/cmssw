...
namespace {
  ...
  *************************************************/
  ...
  class L1TUtmTriggerMenuTest : public Histogram1D<L1TUtmTriggerMenu, SINGLE_IOV> {
  public:
    L1TUtmTriggerMenuTest()
        : Histogram1D<L1TUtmTriggerMenu, SINGLE_IOV>(
              "L1TUtmTriggerMenu test", "L1TUtmTriggerMenu test", 10, 0.0, 10.0) {}

    bool fill() override {
      auto tag = PlotBase::getTag<0>();
      for (auto const& iov : tag.iovs) {
        std::shared_ptr<L1TUtmTriggerMenu> payload = Base::fetchPayload(std::get<1>(iov));
        if (payload.get()) {
          fillWithValue(1.);
          const auto& theMap = payload->getAlgorithmMap();
          for (const auto& [name, algo] : theMap) {
            std::cout << "algo name:" << name << std::endl;
          }
        }  // payload
      }    // iovs
      return true;
  ...
  class L1TUtmTriggerMenuPlot : public PlotImage<L1TUtmTriggerMenu, SINGLE_IOV> {
  public:
    L1TUtmTriggerMenuPlot() : PlotImage<L1TUtmTriggerMenu, SINGLE_IOV>("L1TUtmTriggerMenu plot") {}
  ...
  class L1TUtmTriggerMenu_CompareBase : public PlotImage<L1TUtmTriggerMenu, nIOVs, ntags> {
  public:
    L1TUtmTriggerMenu_CompareBase()
  ...
  using L1TUtmTriggerMenu_Compare = L1TUtmTriggerMenu_CompareBase<MULTI_IOV, 1>;
  using L1TUtmTriggerMenu_CompareTwoTags = L1TUtmTriggerMenu_CompareBase<SINGLE_IOV, 2>;

}  // namespace

PAYLOAD_INSPECTOR_MODULE(L1TUtmTriggerMenu) {
  PAYLOAD_INSPECTOR_CLASS(L1TUtmTriggerMenuTest);
  PAYLOAD_INSPECTOR_CLASS(L1TUtmTriggerMenuPlot);
  PAYLOAD_INSPECTOR_CLASS(L1TUtmTriggerMenu_Compare);
  PAYLOAD_INSPECTOR_CLASS(L1TUtmTriggerMenu_CompareTwoTags);
}
